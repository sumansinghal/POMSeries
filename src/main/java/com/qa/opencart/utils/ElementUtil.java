...
public class ElementUtil {
	...
	// private is used in public - it is called as encapsulation
		...
		public WebElement getElement(By locator){// return webelement
			WebElement ele = driver.findElement(locator);
			if(Boolean.parseBoolean(DriverFactory.highlight));{//if condition cannot take string so convert to boolean
			jsUtil.flash(ele);
		}
			return ele;
		...
		public boolean checkElementDisability(By locator, String attrName) {
			String attrVal = doGetAttributeValue(locator, attrName);
		...
		public boolean checkElementsDeisplayed(By locator) {
		...
		public boolean checkElementsDeisplayed(By locator, int eleOccurence) {
		...
		/************************************
		 * drop down utils select class use webelement
		 **************************/
		...
		public void doSelectDropDown(By locator, String value) {
			Select select = new Select(getElement(locator));
			List<WebElement> optionsList = select.getOptions(); // get options gives all element
		...
		public int rightClicksItemsCount(By rightClick, By items) {
		...
		public List<String> getRightClickList(By rightClick, By items) {
			...
			List<String> itemsValueList = new ArrayList<String>();// creating empty array list
//			WebElement rightClickMe = getElement(rightClick);
			...
			for (WebElement e : menuList) {
				...
				itemsValueList.add(text);// list prints all values
				System.out.println(text);
			}

			return itemsValueList;// return list of strings
	...
	//***********************************wait utils *********************************/
		...
		public WebElement waitForElementPresenceWithWebDriverWait(By locator, int timeOut, int pollingTime) {
			WebDriverWait wait = new WebDriverWait(driver, timeOut);
			wait.withMessage(Error.TIME_OUT_WEB_ELEMENT).pollingEvery(Duration.ofMillis(pollingTime))
		...
		public WebElement waitForElementPresenceWithFluetWait(By locator, int timeOut, int pollingTime) {
			Wait<WebDriver> wait = new FluentWait<WebDriver>(driver).withTimeout(Duration.ofSeconds(timeOut))
					.withMessage(Error.TIME_OUT_WEB_ELEMENT).pollingEvery(Duration.ofMillis(pollingTime))
					.ignoring(StaleElementReferenceException.class, NoSuchElementException.class);
